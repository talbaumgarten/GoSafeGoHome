    ...
    def init_model(self, model_name=None):
        ...
        try:
            ...
            if not self._SERVICE_ACCOUNT_FILE_PATH:
                raise ValueError(
                    "Service account file path is not set. Please set Gemini._SERVICE_ACCOUNT_FILE_PATH or ensure it has a default value.")

            try:
                credentials, _ = google.auth.load_credentials_from_file(self._SERVICE_ACCOUNT_FILE_PATH)
            except FileNotFoundError:
                raise FileNotFoundError(
                    f"Service account file not found at: {self._SERVICE_ACCOUNT_FILE_PATH}. Please check the path.")
