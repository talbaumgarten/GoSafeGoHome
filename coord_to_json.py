...
def coord_to_json(origin, destination, output_path='routes.json'):
    ...
    safe_path = nx.shortest_path(G, orig_node, dest_node, weight=lambda u, v, d: edge_safety_weight(u, v, d))

    fig, ax = ox.plot_graph_routes(G, [shortest_path, safe_path], route_linewidth=6, node_size=0)
    plt.show()


    # Build route geometries
    routes = []
    for idx, path in enumerate([shortest_path, safe_path]):
        coords = []
        for node in path:
            point = G.nodes[node]
...
if __name__ == "__main__":
    origin_address = "Bugrashov Beach Tel Aviv"        # Bugrashov Beach
    destination_address = "Sarona Market Tel Aviv"   # Sarona Market
    output_path = 'routes.json'

    coord_to_json(origin_address, destination_address, output_path)
